<template>
  <div class="nav-menu">
    ...
    >
      ...
      <!--:key="item.value"-->
      <!--:id="item.ref"-->
      <!--v-if="item.children.length > 0 && menuparent(item)"-->
      ...
      <!--:index="val.value"-->
      <!--:key="val.value"-->
      <!--v-hasPermi="[`${val.perms}`]"-->
      <!--v-if="-->
      <!--(val.children.length == 0 && val.label != '客户管理') ||-->
      <!--(val.children.length == 0 &&-->
      <!--val.label === '客户管理' &&-->
      <!--(enterpriseProperty != '01' || orgId == 0))-->
      <!--"-->
      ...
      <!--:key="val.value"-->
      <!--:id="val.ref"-->
      <!--v-if="val.children.length > 0 && menuparent(val)"-->
      <!--:popper-append-to-body="false"-->
      <!--class="submenu-item submenu-children-item"-->
      <!--&gt;-->
      <!--<template slot="title">{{ val.label }}</template>-->
      <!--<template v-for="vals in val.children">-->
      <!--<el-menu-item-->
      <!--:index="vals.value"-->
      <!--:key="vals.value"-->
      <!--v-hasPermi="[`${vals.perms}`]"-->
      <!--v-if="-->
      <!--vals.label != '客户管理' ||-->
      <!--(vals.label === '客户管理' &&-->
      <!--(enterpriseProperty != '01' || orgId == 0))-->
      <!--"-->
      <!--&gt;{{ vals.label }}</el-menu-item-->
      <!--&gt;-->
      <!--</template>-->
      <!--</el-submenu>-->
      <!--</template>-->
...
export default {
  ...
  methods: {
    handleSelect(key, indexId) {
      ...
      this.resetSetItem("indexId", JSON.stringify(indexId));
      this.$store.dispatch(
        "setindexId",
